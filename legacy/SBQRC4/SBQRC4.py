...
class SBClient:
    ...
    def socket_readable_notification(self):
        try:
            ...
            if err.args[0] == errno.EAGAIN or err.args[0] == errno.EWOULDBLOCK:
                print(
                    "[{}:{}] Nonblocking read failed, will retry: {}".format(
                        self.host, self.port, err
                    )
                )
            else:
                print(
                    "[{}:{}] Nonblocking read failed hard, disconnect: {}".format(
                        self.host, self.port, err
                    )
                )
                self.disconnect(err)
    ...
    def socket_writable_notification(self):
        ...
        try:
            ...
            if err.args[0] == errno.EAGAIN or err.args[0] == errno.EWOULDBLOCK:
                print(
                    "[{}:{}] Nonblocking send failed, will retry: {}".format(
                        self.host, self.port, err
                    )
                )
            else:
                print(
                    "[{}:{}] Nonblocking send failed hard, disconnect: {}".format(
                        self.host, self.port, err
                    )
                )
                self.disconnect(err)
    ...
    def message(self, msg):
        print(
            "adding message with length {} to client {}:{} writebuffer.".format(
                len(msg), self.host, self.port
            )
        )
...
class SBQRServer:
    ...
    def qr_forw_to(self, rawdata):
        if rawdata[9:15] == "\xfd\xfc\x1e\x66\x6a\xb2":
            ...
            if (ip, port) in self.hosts:
                ...
                resp += "\x00" * 4
            resp += "".join(
                chr(random.randrange(0, 256)) for _ in range(4)
            )  # random cookie here
            resp += rawdata[9:]
    ...
    def qr_parse03(raw):
        ...
        cooked = [
            (prepared[i], prepared[i + 1]) for i in range(0, len(prepared) - 1, 2)
        ]
        return dict(cooked)
    ...
    def process_qr(self, recv_data, address):
        print("handle qr")
        if (
            recv_data[0] == "\x09" and len(recv_data) >= 5
    ...
    def sb_sendpush02(self, host):
        ...
        if len(localips) == 1:
            ...
            print(
                "WARNING: sb_sendpush02: Multiple localips: {}, using random: {}".format(
                    localips, localip
                )
            )
        ...
        print(
            "Sending info about host {}:{} to {} clients".format(
                host.ip, host.port, len(self.clients)
            )
        )
    ...
    def remove_client(
        self, client, quitmsg
    ):  # cant delete clients inside their own functions?
        del self.clients[client._socket]
    ...
    def run(self):
        ...
        while True:
            (rlst, wlst, _) = select.select(
                [self.qr_socket, self.sb_socket]
                + [x._socket for x in self.clients.values()],
                [x._socket for x in self.clients.values() if x.write_queue_size() > 0],
                [],
                15,
            )
            for x in rlst:
...
def main():
    ...
    parser.add_argument(
        ...
        dest="fakehosts",
        action="store_true",
        help="Add fake hosts for testing.",
    )
